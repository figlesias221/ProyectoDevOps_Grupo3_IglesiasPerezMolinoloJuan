...
export class MakeReservationComponent implements OnInit {
  ...
  public makeReservation(): void {
    ...
    if (!this.displayError) {
      ...
      this.reservationService.makeReservation(this.reservationIntent).subscribe(
        (confirmation) => this.loadConfirmation(confirmation),
  ...
  private validateName(): void {
    ...
    if (this.name == null || !regex.test(this.name)) {
      ...
      this.errorMessages.push(
        'El nombre ingresado es invalido - solo caracteres alfabeticos'
      );
    }
  }
  ...
  private validateSurname(): void {
    ...
    if (this.surname == null || !regex.test(this.surname)) {
      this.displayError = true;
      this.errorMessages.push(
        'El apellido ingresado es invalido - solo caracteres alfabeticos'
      );
    }
  }
  ...
  private validateEmail(): void {
    if (this.email == null || !isEmailValid(this.email)) {
      this.displayError = true;
      this.errorMessages.push(
        'Es necesario especificar un email y que este sea válido'
      );
    }
  }
